...
# 사용법: ./scripts/resume_poa.sh [노드 수]
#
# 예시:
#   ./scripts/resume_poa.sh 3      # 3개 노드로 재시작
#   ./scripts/resume_poa.sh 9      # 9개 노드로 재시작
...
for i in $(seq 1 $NODE_COUNT); do
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        echo -e "${RED}    먼저 ./scripts/start_poa.sh $NODE_COUNT 를 실행하여 초기 설정을 생성하세요.${NC}"
        exit 1
    fi
done
...
if [ "$DB_EXISTS" = false ]; then
    ...
    echo -e "${RED}    먼저 ./scripts/start_poa.sh $NODE_COUNT 를 실행하여 제네시스 블록을 생성하세요.${NC}"
    exit 1
fi
echo "  ✓ DB 존재 확인"
